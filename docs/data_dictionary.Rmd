...
This dataset combines HUMC legacy literature search data and HMH shared-form literature search data for analysis of request volume, campus patterns, requestor groups, purposes, workload, and research topics.
...
This dataset reshapes request purposes so that each request-purpose pairing appears as its own row.
...
This dataset contains one row per request-topic lemma after text cleaning, phrase collapsing, lemmatization, stop-word removal, and custom merging.
...
This dataset identifies possible multi-word phrases that may be useful additions to `phrases.csv`.
...
- HUMC legacy logs and HMH shared-form data have different source structures, so some variables are only available for one source.
- `Unknown/Not specified` indicates missing, blank, or unmapped values.
- Raw fields such as `requestor_category_raw` are preserved for auditability.