...
check_campus_flags <- humc_master %>%
  dplyr::mutate(
    n_campus_flags = humc + carrier + jfk + palisades + network
  ) %>%
  dplyr::count(
    n_campus_flags,
    humc,
...
readr::write_csv(
  check_by_file,
  file.path(processed_dir, paste0("humc_import_check_by_file_", run_timestamp, ".csv"))
)

readr::write_csv(
  check_missing_dates,
  file.path(processed_dir, paste0("humc_missing_dates_", run_timestamp, ".csv"))
)

readr::write_csv(
  check_missing_topics,
  file.path(processed_dir, paste0("humc_missing_topics_", run_timestamp, ".csv"))
)

readr::write_csv(
  check_year_mismatch,
  file.path(processed_dir, paste0("humc_year_mismatch_", run_timestamp, ".csv"))
)

readr::write_csv(
  check_campus_flags,
  file.path(processed_dir, paste0("humc_campus_flag_check_", run_timestamp, ".csv"))
)

readr::write_csv(
  check_requestor_flags,
  file.path(processed_dir, paste0("humc_requestor_flag_check_", run_timestamp, ".csv"))
)

readr::write_csv(
  check_purpose_flags,
  file.path(processed_dir, paste0("humc_purpose_flag_check_", run_timestamp, ".csv"))
)
