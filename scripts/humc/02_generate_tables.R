...
#
# Purpose:
#   Load outputs/humc/out.rds and generate CSV, RDS, and Excel workbook
#   outputs for the HUMC text/category/purpose/citation analysis.
...
# Clear old outputs instead of archiving
clear_output_folder(csv_dir, "\\.(csv|rds)$")
clear_output_folder(output_dir, "^summary_report_.*\\.xlsx$|^humc_summary_report\\.xlsx$")
...
refresh_top_lemmas <- function(out, top_n = 300) {
  out$data_norm %>% count(lemma, sort = TRUE) %>% slice_head(n = top_n)
}
...
refresh_category_purpose <- function(out) {
  ...
  list(category_by_purpose = category_by_purpose, category_by_person_purpose = category_by_person_purpose)
}
...
refresh_citation_summaries <- function(out) {
  ...
  request_category_citations <- out$data_norm %>% filter(!is.na(category)) %>% distinct(request_id, category, citation_count)
  citation_by_category <- request_category_citations %>%
    ...
    mutate(across(c(mean_citations, median_citations, max_citations), ~ if_else(is.infinite(.x), NA_real_, .x)))
  citation_by_purpose <- out$my_data2 %>%
    ...
    mutate(
      ...
      across(c(mean_citations, median_citations, max_citations), ~ if_else(is.infinite(.x), NA_real_, .x))
    )
  list(citation_by_category = citation_by_category, citation_by_purpose = citation_by_purpose)
}

safe_chisq_analysis <- function(table_matrix) {
  if (nrow(table_matrix) < 2 || ncol(table_matrix) < 2 || sum(table_matrix) == 0) {
    return(list(chi_result = NA, residuals = tibble(), cramers_v = NA_real_))
  }
  chi_result <- suppressWarnings(chisq.test(table_matrix))
  residuals <- as.data.frame(chi_result$stdres) %>%
    rownames_to_column("row_name") %>%
    pivot_longer(cols = -row_name, names_to = "column_name", values_to = "std_residual")
  n <- sum(table_matrix)
  r <- nrow(table_matrix)
  c <- ncol(table_matrix)
  cramers_v <- sqrt(as.numeric(chi_result$statistic) / (n * min(r - 1, c - 1)))
  list(chi_result = chi_result, residuals = residuals, cramers_v = cramers_v)
}

refresh_category_chi <- function(out) {
  data_norm_chi <- out$data_norm %>% mutate(submitter_type_chi = if_else(submitter_type == "Committee", "OtherProvider", submitter_type))
  submitter_counts <- out$data_norm %>% distinct(request_id, submitter_type) %>% count(submitter_type)
  ...
  if (nrow(chi_table) == 0) {
    return(list(submitter_counts = submitter_counts, chi_table = matrix(numeric(0), nrow = 0), chi_result = NA, chi_residuals = tibble(), cramers_v = NA_real_))
  }
  chi_matrix <- chi_table %>% column_to_rownames("submitter_type_chi") %>% as.matrix()
  chi <- safe_chisq_analysis(chi_matrix)
  list(
    submitter_counts = submitter_counts,
    chi_table = chi_matrix,
    chi_result = chi$chi_result,
    chi_residuals = chi$residuals %>% rename(submitter_type = row_name, category = column_name),
    cramers_v = chi$cramers_v
  )
}

refresh_purpose_analysis <- function(out) {
  my_data2_purpose <- out$my_data2 %>% mutate(submitter_type = if_else(submitter_type == "Committee", "OtherProvider", submitter_type))
  purpose_counts <- out$purpose_long %>% count(purpose, sort = TRUE)
  purpose_by_person <- out$purpose_long %>%
    left_join(my_data2_purpose %>% dplyr::select(request_id, submitter_type), by = "request_id") %>%
    count(submitter_type, purpose, sort = TRUE)
  purpose_table <- purpose_by_person %>% pivot_wider(names_from = purpose, values_from = n, values_fill = 0)
  if (nrow(purpose_table) == 0) {
    return(list(purpose_counts = purpose_counts, purpose_by_person = purpose_by_person, purpose_table = matrix(numeric(0), nrow = 0), purpose_chi = NA, purpose_residuals = tibble(), cramers_v_purpose = NA_real_))
  }
  purpose_matrix <- purpose_table %>% column_to_rownames("submitter_type") %>% as.matrix()
  ...
  list(
    ...
    purpose_residuals = chi$residuals %>% rename(submitter_type = row_name, purpose = column_name),
    cramers_v_purpose = chi$cramers_v
  )
}

refresh_phrase_candidates <- function(out) {
  if (!is.null(out$phrase_results)) return(out$phrase_results)
  list(phrase_candidates = tibble(), phrases_to_add = tibble())
}
...
phrase_results <- refresh_phrase_candidates(out)
...
tables_to_export_clean <- tables_to_export[map_lgl(tables_to_export, ~ is.data.frame(.x) && ncol(.x) > 0)]
write_pretty_workbook(tables = tables_to_export_clean, path = summary_filepath)
...
message("\nCSV/RDS files saved to: ", csv_dir)
