...
#
# Standalone analysis of HMH Article/Chapter requests. This script loads
# the raw HMH data, filters for article requests, and generates a full
...
# Define output paths
paths <- make_output_paths("hmh_article_requests")

# --- 2. Load and Process Data ---
hmh_dat <- read_csv(hmh_path) %>%
  janitor::clean_names() %>%
  filter(select_question_request_type == "Article / Chapter Request") %>%
  mutate(
...
p_by_campus <- summary_tables$`Requests by Campus` %>%
  ...
  coord_flip() +
  labs(title = "HMH Article Requests by Campus", x = NULL, y = "Number of Requests") +
  theme_project()
ggsave(file.path(paths$figures_dir, "hmh_article_req_by_campus.png"), p_by_campus, width = 9, height = 6, dpi = 300)
...
p_by_requestor <- summary_tables$`Requests by Requestor` %>%
  ...
  geom_col(fill = "#E69F00") +
  coord_flip() +
  labs(title = "HMH Article Requests by Requestor", x = NULL, y = "Number of Requests") +
  theme_project()
ggsave(file.path(paths$figures_dir, "hmh_article_req_by_requestor.png"), p_by_requestor, width = 9, height = 7, dpi = 300)
...
purrr::iwalk(
  summary_tables,
  ~ write_archived_csv(df = .x, filename = paste0("article_req_", janitor::make_clean_names(.y)), csv_dir = paths$csv_dir)
)
write_archived_workbook(
  tables = summary_tables,
  path = file.path(paths$output_dir, "hmh_article_request_summary.xlsx")
)

cat("--- HMH Article Request Analysis Complete ---\n")
