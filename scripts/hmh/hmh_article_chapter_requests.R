# Article/Chapter Request analysis ----------------------------------------
...
parse_timestamp <- function(x) {
  dplyr::coalesce(
    suppressWarnings(lubridate::mdy_hms(x, quiet = TRUE)),
    suppressWarnings(lubridate::mdy_hm(x, quiet = TRUE)),
    suppressWarnings(lubridate::ymd_hms(x, quiet = TRUE)),
    suppressWarnings(lubridate::ymd_hm(x, quiet = TRUE))
  )
}
...
hmh_data <- hmh_raw %>%
  mutate(
    ...
    request_received = clean_text_na(how_was_the_question_request_received),
    campus_affiliation = clean_text_na(campus_affiliation),
    select_question_request_type = clean_text_na(select_question_request_type),
    
    article_chapter_other_source_text = clean_text_na(
      number_of_articles_chapters_retrieved_from_other_include_source
    ),
    article_chapter_other_source_category = classify_article_source(
      article_chapter_other_source_text
    ),
    
    n_article_chapter_other = clean_num_na(
      number_of_articles_chapters_retrieved_from_other_include_source
    ),
    n_article_chapter_subscribed = clean_num_na(
      number_of_articles_chapters_retrieved_from_subscribed_content
    ),
    n_article_chapter_docline = clean_num_na(
      number_of_articles_chapters_retrieved_from_docline
    ),
...
# Filter to Article/Chapter Requests ---------------------------------------
...
article_chapter_requests_export <- article_chapter_data %>%
  transmute(
    request_id,
    timestamp,
    requestor_role,
...
tables_to_export <- list(
  ...
  "Other Source Summary" = article_chapter_other_source_summary,
  "Other Source Detail" = article_chapter_other_source_detail,
  "Request Level Export" = article_chapter_requests_export
)
...
gt_article_requestor <- article_chapter_by_requestor %>%
  gt() %>%
  tab_header(title = paste0("Article/Chapter Requests by Requestor Role (", analysis_year, ")")) %>%
  fmt_percent(columns = prop, decimals = 1)

gtsave(
  gt_article_requestor,
  file.path(formatted_tables_dir, "article_chapter_by_requestor.html")
)
...
p_article_by_month <- ggplot(article_chapter_by_month, aes(x = request_month, y = n_requests)) +
  ...
  scale_x_date(
    ...
    date_breaks = "1 month",
    limits = as.Date(c(
      paste0(analysis_year, "-01-01"),
      paste0(analysis_year, "-12-31")
    ))
  ) +
  labs(
...
ggsave(file.path(figures_dir, "article_requests_by_requestor.png"), p_article_by_requestor, width = 8, height = 5, dpi = 300)
ggsave(file.path(figures_dir, "article_requests_by_received.png"), p_article_by_received, width = 8, height = 5, dpi = 300)
ggsave(file.path(figures_dir, "article_requests_by_campus.png"), p_article_by_campus, width = 8, height = 5, dpi = 300)
ggsave(file.path(figures_dir, "article_requests_by_month.png"), p_article_by_month, width = 9, height = 5, dpi = 300)
ggsave(file.path(figures_dir, "article_request_source_totals.png"), p_article_source_totals, width = 8, height = 5, dpi = 300)
ggsave(file.path(figures_dir, "article_request_source_mix_by_requestor.png"), p_article_source_by_requestor, width = 10, height = 6, dpi = 300)
ggsave(file.path(figures_dir, "article_request_total_per_request_histogram.png"), p_article_total_per_request, width = 8, height = 5, dpi = 300)

# Write summary workbook last ---------------------------------------------
...
cat("\nNewest article request summary workbook written to:\n", summary_filepath, "\n")
